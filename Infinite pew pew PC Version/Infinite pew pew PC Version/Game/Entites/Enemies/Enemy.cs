    ...
    public abstract class Enemy : Entity
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            else
            {
                ...
                this.Color = Color.White * (1 - timeUntilStart / 30f);
            }
        ...
        public virtual void Kill()
        {
            ...
            EntityManager.Add(new FadingOutMessage(this.Position, this.PointValue + "", 30));
            ...
            if(!GameplayScreen.GameTime.IsRunningSlowly)
                for (int i = 0; i < 20; i++)
                {
                    ...
                    GameplayScreen.ParticleManager.CreateParticle(EntityManager.Resources.Particle, this.Position, color, 35f, 1.5f, state);
                }   
        ...
        protected IEnumerable<int> FollowFasterWhenFar()
        {
            ...
            {
                if ((EntityManager.PlayerShip.Position - this.Position).Length() > 1f)
                    this.Velocity += (EntityManager.PlayerShip.Position - this.Position) * 0.015f;
                //this.Velocity += Vector2.Normalize(EntityManager.PlayerShip.Position - this.Position) * acceleration;
                if (this.Velocity != Vector2.Zero)
                    this.Orientation = Velocity.ToAngle();
        ...
        protected IEnumerable<int> MoveRandomly()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    Velocity += MathUtil.FromPolar(direction, 0.8f);
        ...
        protected IEnumerable<int> FollowAndSpin(float acceleration, float spinAcceleration)
        {
            ...
            while (true)
            {
                if ((EntityManager.PlayerShip.Position - this.Position).Length() > 1f)
                    this.Velocity += (EntityManager.PlayerShip.Position - this.Position).ScaleTo(acceleration);
                //this.Velocity += Vector2.Normalize(EntityManager.PlayerShip.Position - this.Position) * acceleration;
                this.Orientation -= spinAcceleration * GameplayScreen.SlowMoMultiplier;
        ...
        protected IEnumerable<int> BounceAndSpin(float acceleration)
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    Orientation -= 0.3f * GameplayScreen.SlowMoMultiplier;
                    yield return 0;
                }
            }
        }
