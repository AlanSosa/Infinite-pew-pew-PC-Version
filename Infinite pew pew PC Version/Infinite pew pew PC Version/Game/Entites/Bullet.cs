    ...
    public class Bullet:Entity
    {
        ...
        public override void Update(Microsoft.Xna.Framework.GameTime gametime)
        {
            ...
            if (!GameplayScreen.World.Bounds.Contains(this.Position.ToPoint()))
            {
                if (bouncing)
                {
                    this.Velocity = Velocity.ScaleTo(-50);
                    bounceTimes--;
                    if (bounceTimes <= 0)
                        IsExpired = true;
                }
            ...
            if (IsExpired)
            {
                if(!GameplayScreen.GameTime.IsRunningSlowly)
                    for (int i = 0; i < 5; i++)
                        GameplayScreen.ParticleManager.CreateParticle(EntityManager.Resources.Particle, Position, Color.LightBlue, 20, 1,
                            new ParticleState()
                            {
                                Velocity = rand.NextVector2(0, 25),
